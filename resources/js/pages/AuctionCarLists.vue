...
<div>
      ...
      <div v-for="(car, idx) in cars.slice().reverse()" :key="idx" v-if="idx <= loopCondition" class="col-md-3 mobile-div">
        ...
        <div class="listing-car-item-meta">
          <div class="car-meta-top">
            <div class="car-title">
              <span v-if="itemsInfo.ID == 123" v-for="(itemsInfo, itemsIndex) in car.INFO" :key="itemsIndex">{{itemsInfo.VERDI}}
                <span v-if="itemsInfo.ID == 129" v-for="(itemsInfo, itemsIndex) in car.INFO" :key="itemsIndex">{{itemsInfo.VERDI}}</span>
              </span>
            </div>
            <div class="price">
  ...
  <div v-else style="margin-top: 20px; margin-bottom: 20px;">
    <!-- <LoadingBar v-else/> -->
    Auksjon fra iAuto AS er avsluttet.
    Vi takker for budene - alle får svar på budene som er lagt inn via e-post.
    Neste auksjon er planlagt og e-post med informasjon blir sendt når bilene er klare.
  </div>
</div>
</template>
